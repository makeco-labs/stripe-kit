...
export function findEnvDirectories(cwd: string = process.cwd()): string[] {
  ...
  if (gitRoot && fs.existsSync(gitRoot) && gitRoot !== cwd && gitRoot !== workspaceRoot) {
 ...
 */
export function getEnvFilePatterns(env: string): string[] {
  const patterns: Record<string, string[]> = {
    dev: ['.env.dev.local', '.env.development.local', '.env.dev', '.env.development'],
    test: ['.env.test.local', '.env.test'],
    staging: ['.env.staging.local', '.env.staging'],
    prod: ['.env.prod.local', '.env.production.local', '.env.prod', '.env.production'],
  };
